...
d-i preseed/late_command string in-target usermod -G contestant contestant; mkdir /target/root/.ssh; echo "      dhcp-identifier: mac" >> /target/etc/netplan/01-netcfg.yaml; mkdir /target/root/.ssh; chmod 700 /target/root/.ssh; wget http://<?php echo env("SYS_URL"); ?>/proxy/pixie/firstboot -O /target/etc/rc.local; chmod +x /target/etc/rc.local